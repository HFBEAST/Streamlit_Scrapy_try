...
def get_df_ec():
    ...
    for item in items:
        datum_ec = {}
        datum_ec['title'] = item.find('p', {'class': 'items-grid_itemTitleText_009d5fa0'}).text
        price = item.find('p', {'class': 'items-grid_price_009d5fa0'}).text
        datum_ec['price'] = int(price.replace('¥', '').replace(',', ''))
        datum_ec['link'] = item.find('a')['href']
        is_stock = item.find('p', {'class': 'items-grid_itemTitleText_009d5fa0'})  == None
